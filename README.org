...
The code below demonstrates basic pipeline parallelism in PyTorch without using any third-party libraries. However, it's important to note that this example does not include features such as multi-device support, automatic model partitioning, and load balancing etc. In this example, we split SimpleNet into four pipeline stages. We modify the forward method to accept a stage argument, which determines which stage is being computed. During training, we first compute the forward pass for stage 1 and store the output. We then detach the output from the computational graph and reattach gradients to it. This allows us to perform the backward pass for stage 2 independently, followed by the backward pass for stage 1.

In this pipeline setup, a "bubble" occurs when a stage is idle, waiting for input from the previous stage or for the next stage to be available to receive output. Bubbles can lead to inefficiencies in the pipeline, as they result in idle time for the involved stages. In practice, techniques such as pipeline schedule optimization can be used to minimize the impact of bubbles and improve overall pipeline efficiency.
...
Actually there is =torch.distributed.pipeline.sync.Pipe= instead of =Pipe= from Python's =multiprocessing=. The =Pipe= module from =torch.distributed= automatically handles the pipeline stages, data transfer between stages, and synchronization. The =chunks= parameter is set to the number of accumulation steps, which controls the number of microbatches used in the pipeline.

#+begin_src python :tangle src/basic-pipeline-parallelism-iii.py :comments link
  import os
  import torch
  import torch.nn as nn
  import torch.optim as optim
  ...
  # Initialize the model and wrap it with Pipe
  model = SimpleNet()
  model = Pipe(model, chunks=accumulation_steps, checkpoint="never").to(device)
...
Please note that this example assumes a single device for simplicity. In practice, you can combine =torch.distributed.pipeline.sync.Pipe= with =torch.distributed= for multi-device and distributed training.
...
While model parallelism looks at execution over multiple GPUs to distribute memory demands, some systems attempt to make use of main system memory (DRAM) rather than horizontally scaling across more GPUs.

- Memory Offloading

  1. Graph-based swapping and upfront offload planning

     Initially many works treated offloading as a “swapping” problem — deciding when to swap tensors off of GPU memory and onto DRAM. Most use graph analysis algorithms to determine where to “inject” a swap operation based on when an activation, gradient, or parameter might next be used in the execution graph. SwapAdvisor [cite:@inproceedings], the most advanced of these swapping systems, uses a parallelized genetic search algorithm to analyze where the swap operators should be placed for best performance. It was also one of the first systems to support offloading parameters as well as activations, which is critical for training billion-parameter model architectures.

  2. Dynamic offloading

     Another approach was proposed with ZeRO-R [cite:@zero2019], a system for offloading that sends activations and parameters to DRAM dynami- cally. This approach “offloads when needed”, rather than planning offloads up front. The irregularity of the design can introduce issues such as memory fragmentation, but it adds a great deal of flexibility versus graph-based designs.

  3. "Independent block" strategy

     Hydra [cite:@hydra2021] opts for an “independent block” strategy, dividing a model architecture into submodels (like model parallelism) that can then be spilled between DRAM and GPU memory freely. An analogy can be drawn to spilling in RDBMSs, where independent data chunks can be sent down to a lower level of memory. Unlike other spilling systems, Hydra’s execution pattern is identical to model parallelism, and separates the execution of each model shard entirely.

     L2L [cite:@l2l2020] uses a design similar to Hydra’s but is more restricted in its sharding approach. It targets Transformer architectures specifically, and swaps self-attention blocks (standard Transformer op- erators) with heuristics selected specifically for its target class of models. This allows it to perform very well on Transformer architectures, but prevents it from achieving the flexibility of Hydra or the dynamic generality of ZeRO-R.

    The only option for hybrid-device execution on wide operators is to either serialize the parallel operator (index lookup in the table case) and rewrite the series of operations into a deep, rather than wide, model, or else to actually execute the wide operator on the CPU.

- Computation Offloading

  Mixed-CPU-GPU compute. ZeRO [cite:@zerooffload2021] proposed running parameter updates on the CPU while GPU execution is ongoing, specifically for the popular Adam optimizer.

  Embedding tables are very wide memory-intensive operators which generally feed into some smaller DNN for further processing. Without any optimiza- tion, the sheer scale of the embedding table would force CPU-only execution. Alternatively, a user could place the embedding table on the CPU while the DNN sits in GPU memory and enjoys the ben- efits of GPU acceleration. Some works such as Hotline [cite:@hotline2022] try and pipeline data through the model, from the CPU-based embedding table into the GPU-accelerated DNN.

**** Hybrid parallelism

Hybrid parallelism techniques can be classified into two broad categories — “true” hybrids that integrate parallelization techniques from the ground up [fn:29], and top-down hybrids that select between different strategies at different stages of execution.

***** Ground-up

- Hydra's approach: using a spilling technique to reduce the number of GPUs needed for scalable model-parallel training, and then applying a layer of task parallelism on top to support efficient multi-model training.
- Fully Sharded Data Parallelism (FSDP), originally introduced with ZeRO [cite:@zero2019], offers a hybrid of model parallelism and data parallelism.
- ZeRO-Offload [cite:@zerooffload2021] proposed combining FSDP with spilling per accelerator, offloading sharded layer parameters that will not be used in the near future.
- 3D Parallelism [cite:@zero2019; @megatron2019;@song2023optimuscc;] combines 1) FSDP with 2) pipeline parallelism and 3) tensor parallelism, to exploit scalable data parallelism along with parallel depth-wise and width-wise sharded execution.
- Strategy finding: automate the process of combining parallelization techniques within a model.
  + FlexFlow [cite:@flexflow2018]
  + Alpa [cite:@alpa2022]
- Model-Data Parallelism for Recommender Models (DLRM): [cite:@dlrm2019;@https://doi.org/10.48550/arxiv.2205.04702;@https://doi.org/10.48550/arxiv.1906.03109;@https://doi.org/10.48550/arxiv.2302.03525;] [fn:30]
  #+begin_quote
  Our parallelized DLRM will use a combination of model parallelism for the embeddings and data parallelism for the MLPs to mitigate the memory bottleneck produced by the embeddings while parallelizing the forward and backward propagations over the MLPs. Combined model and data parallelism is a unique requirement of DLRM as a result of its architecture and large model sizes.
  #+end_quote
...
The most general and commonly used model compression method [fn:4]. Quantization [cite:@Zhao_2021; @zandonati2023optimal; @gholami2021survey; @rokh2023comprehensive; @liang2021pruning; @bhandare2019efficient; @kuzmin2023pruning;@https://doi.org/10.48550/arxiv.2206.09557;] reduces a model’s size by using fewer bits to represent its parameters.
...
[fn:23] [cite:@chen2022understanding;@fedus2022review;@Singh_2023;@rajbhandari2022deepspeedmoe;]
** Finetuning
...
- Feature store [fn:22]
* TODO ML in edge computing
[cite:@2022]
* Footnotes
[fn:30] [[https://github.com/NVIDIA/DeepLearningExamples/tree/master/PyTorch/Recommendation/DLRM][DeepLearningExamples/PyTorch/Recommendation/DLRM at master · NVIDIA/DeepLearningExamples]]

[fn:29] [[https://www.microsoft.com/en-us/research/blog/deepspeed-extreme-scale-model-training-for-everyone/][DeepSpeed: Extreme-scale model training for everyone - Microsoft Research]]; [[https://www.deepspeed.ai/tutorials/pipeline/][Pipeline Parallelism - DeepSpeed]]
...
[fn:4] Using 16 bits to represent a float is called half precision. Instead of using floats, you can have a model entirely in integers; each integer takes only 8 bits to represent. Useful links: [[https://www.mathworks.com/company/newsletters/articles/what-is-int8-quantization-and-why-is-it-popular-for-deep-neural-networks.html][What Is int8 - Quantization and Why Is It Popular for Deep Neural Networks? - MATLAB & Simulink; ]][[https://huggingface.co/docs/optimum/concept_guides/quantization][Quantization]]; [[https://www.kaggle.com/code/anilkrsah/model-quantization][model quantization | Kaggle]]; [[https://www.kaggle.com/code/ritvik1909/model-quantization][Model Quantization | Kaggle]]
